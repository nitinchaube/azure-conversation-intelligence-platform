...
@description('Optional. SQL administrator login. Used as fallback when Entra-only auth is restricted in the subscription.')
param sqlAdministratorLogin string = 'sqladmin'

@secure()
@description('Optional. SQL administrator password. Used as fallback when Entra-only auth is restricted.')
param sqlAdministratorLoginPassword string = 'Ckm@${uniqueString(resourceGroup().id, deployment().name)}!1Aa'

@description('Optional. Location for App Service Plan. New PAYG subscriptions may have zero App Service quota in many regions. Set to a region with available quota.')
...
module sqlDBModule 'br/public:avm/res/sql/server:0.21.1' = {
  ...
  params: {
    ...
    // NOTE: Changed from azureADOnlyAuthentication: true to hybrid auth.
    // New Pay-As-You-Go subscriptions often block AAD-only SQL provisioning
    // (surfaces as misleading 'ProvisioningDisabled' error). Managed Identity
    // auth still works because the AAD admin is preserved below.
    administrators: {
    ...
    databases: [
      {
        ...
        // Basic tier: 2GB max (plenty for demo/resume project), cheapest,
        // and universally available. Avoids Serverless SKU gating in new subs.
...
module webServerFarm 'br/public:avm/res/web/serverfarm:0.5.0' = {
  ...
  params: {
    ...
    kind: 'linux'
    diagnosticSettings: enableMonitoring ? [{ workspaceResourceId: logAnalyticsWorkspaceResourceId }] : null
    // B1: cheapest dedicated Linux tier. Deployed to appServiceLocation (westeurope)
    // because the primary region has zero App Service quota on new PAYG subs.
    skuName: 'B1'
    skuCapacity: 1
    zoneRedundant: false
  }
