...
def test_km_generic_golden_path_refactored(login_logout, request):
    ...
    request.node._nodeid = "13477 - Golden Path - KM Generic - Telecom - Test golden path demo script works properly"
    ...
    try:
        ...
        if failed_questions:
            logger.warning(f"Golden path test completed with {len(failed_questions)} failed questions out of {len(telecom_questions)} total")
