...
import time
from config.constants import telecom_questions
import io
...
def test_user_filter_functioning(login_logout, request):
    ...
    request.node._nodeid = "13480 - KM Generic - Telecom - Validate filter functionality should work as per filtered data selected"
...
def test_after_filter_functioning(login_logout, request):
    ...
    request.node._nodeid = "13482 - KM Generic - Telecom - Validate after applying filter charts/graphs should show filtered data"
...
def test_hide_dashboard_and_chat_buttons(login_logout, request):
    ...
    # Set custom test name for pytest HTML report
    request.node._nodeid = "13485 - KM Generic - Telecom - Validate Hide Dashboard and Hide Chat buttons"
...
def test_refine_chat_chart_output(login_logout, request):
    ...
    # Set custom test name for pytest HTML report
    request.node._nodeid = "13526 - US_12962_KM Generic - Telecom - Improve Chart Generation Experience in Chat"
...
def test_chat_greeting_responses(login_logout, request):
    ...
    # Set custom test name for pytest HTML report
    request.node._nodeid = "20426 - US_20054_KM Generic - Telecom - Greeting related experience in Chat"
...
def test_chat_history_panel(login_logout, request):
    ...
    # Set custom test name for pytest HTML report
    request.node._nodeid = "13483 - KM Generic - Telecom - Validate Chat History- user able to edit, save, delete and delete all chat history"
    ...
    try:
        ...
        for i, question in enumerate(telecom_questions, start=1):
        ...
        if failed_questions:
            logger.warning(f"Chat history test completed with {len(failed_questions)} failed questions out of {len(telecom_questions)} total")
...
def test_citation_panel_closes_with_chat(login_logout, request):
    ...
    # Set custom test name for pytest HTML report
    request.node._nodeid = "18433 - KM Generic - Telecom - Citation panel should close after hiding chat"
